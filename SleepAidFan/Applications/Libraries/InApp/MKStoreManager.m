...
@property (nonatomic, copy) void (^onTransactionCompleted)(NSString *productId, NSData* receiptData,SKPaymentTransaction* transaction);
...
- (void)productsRequest:(SKProductsRequest *)request didReceiveResponse:(SKProductsResponse *)response
{
	...
	for(NSString *invalidProduct in response.invalidProductIdentifiers)
		NSLog(@"Problem in iTunes connect configuration for product: %@", invalidProduct); //Will get called for ipad products when using iphone and vice versa
#endif
...
- (void) buyFeature:(NSString*) featureId
         onComplete:(void (^)(NSString*, NSData*,SKPaymentTransaction*)) completionBlock
...
-(void) addToQueue:(NSString*) productId
{
  if ([SKPaymentQueue canMakePayments])
  {
    ...
    if(index == NSNotFound)
    {
        UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"Purchase not performed"
                                                        message:@"The purchase could not be perfomed. Please try again later"
                                                       delegate:nil
        ...
        {
            self.onTransactionCancelled(productId);
        }
        return;
    }
    ...
    [subscriptionProduct verifyReceiptOnComplete:^(NSNumber* isActive)
     {
       ...
       if(self.onTransactionCompleted)
         self.onTransactionCompleted(productIdentifier, receiptData,transaction);
     }
                                         onError:^(NSError* error)
     {
...
-(void)reloadProductsifNeeded
{
    if (_sharedStoreManager.purchasableObjects!=nil && _sharedStoreManager.purchasableObjects.count==0)
    {
        [_sharedStoreManager requestProductData];
    }
}
